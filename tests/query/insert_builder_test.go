	...
	tests := []struct {
	...
	}{
		{
			...
			func() *query.InsertBuilder {
				return query.NewInsertBuilder(&db.MySQLQueryBuilder{}, cache.NewAsyncQueryCache(100)).
					Table("users").
		...
		},
		{
			...
			func() *query.InsertBuilder {
				return query.NewInsertBuilder(&db.MySQLQueryBuilder{}, cache.NewAsyncQueryCache(100)).
					Table("users").
					InsertUsing([]string{"name", "age"}, query.NewBuilder(&db.MySQLQueryBuilder{}, cache.NewAsyncQueryCache(100)).
						Table("profiles").
						Select("name", "age").
