...
func TestWhereSelectBuilder(t *testing.T) {
	tests := []struct {
		...
		},
		{
			"WhereExists",
			func() *query.Builder {
				return query.NewBuilder(mysql.NewMySQLQueryBuilder(), cache.NewAsyncQueryCache(100)).WhereExists(func(b *query.Builder) {
		...
		},
		{
			"OrWhereExists",
			func() *query.Builder {
				return query.NewBuilder(mysql.NewMySQLQueryBuilder(), cache.NewAsyncQueryCache(100)).Where("city", "=", "New York").OrWhereExists(func(b *query.Builder) {
					b.Select("id").Table("users").Where("name", "=", "John")
				})
			},
		...
		},
		{
			"WhereNotExists",
			func() *query.Builder {
				return query.NewBuilder(mysql.NewMySQLQueryBuilder(), cache.NewAsyncQueryCache(100)).WhereNotExists(func(b *query.Builder) {
					b.Select("id").Table("users").Where("name", "=", "John")
				})
			},
		...
		},
		{
			"OrWhereNotExists",
			func() *query.Builder {
				return query.NewBuilder(mysql.NewMySQLQueryBuilder(), cache.NewAsyncQueryCache(100)).Where("city", "=", "New York").OrWhereNotExists(func(b *query.Builder) {
					b.Select("id").Table("users").Where("name", "=", "John")
				})
			},
